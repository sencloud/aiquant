...
//  3. 调用 chat.Service.RunCollect（带 30 工具的 tool calling loop），
//     扣费由 chat.Service 统一以 reason=consume_ding + ref=ding_run/<uuid>/<ts>
//     的形式完成，避免 DING runner 与 chat 双重扣费。
//  4. 写一条 Run + Notification（push_status=pending），由 pusher 进程接力推送。
//
...
// NewRunner 构造 DING 调度器。ledgerRepo 暂不直接使用（DING 自身不再扣费），
// 但保留参数以便未来加"任务级单次预算上限"等校验逻辑。
func NewRunner(st *store.Store, exec ChatExecutor, _ *billing.LedgerRepo, l *zerolog.Logger, cfg RunnerConfig) *Runner {
...
func (r *Runner) runOne(ctx context.Context, t *Task) error {
	...
	if err != nil {
		if errors.Is(err, chat.ErrInsufficientBalance) {
	...
	return r.recordSuccess(ctx, t, startedAt, res.FinalText, res.ToolCalls, res.Credits)
}
